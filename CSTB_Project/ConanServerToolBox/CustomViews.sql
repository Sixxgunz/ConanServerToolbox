...
CREATE TABLE cust_stat_xref ( stat_id INTEGER, stat_name TEXT, stat_type INTEGER );
INSERT INTO `cust_stat_xref` (stat_id,stat_name,stat_type) VALUES (19,'Agility',0),
 (17,'Strength',0),
 (14,'Vitality',0),
 (18,'Accuracy',0),
 (15,'Grit',0),
...
CREATE TABLE cust_item_xref ( template_id INTEGER, name TEXT, type TEXT );
INSERT INTO `cust_item_xref` (template_id,name,type) VALUES (10001,'Stone','ingredient'),
 ...
 (11103,'Fragment of Power','ingredient'),
 (51205,'xx_unarmed left','default_game'),
 (51204,'xx_unarmed right','default_game'),
 (6170,'Exceptional Pictish Wizard Mask','armor'),
 (6171,'Exceptional Pictish Wizard Robe','armor'),
 (6172,'Exceptional Pictish Wizard Wraps','armor'),
 (6173,'Exceptional Pictish Wizard Skirt','armor'),
 (6174,'Exceptional Pictish Wizard Moccasins','armor');
DROP VIEW IF EXISTS `Thralls_Placed_In_World`;
CREATE VIEW `Thralls_Placed_In_World` AS SELECT CASE WHEN class LIKE '%human%' THEN ap.id ELSE class END 'Thrall_ID',CASE WHEN class LIKE '%human%' THEN ap.class ELSE class END 'Thrall_Class', 'TeleportPlayer ' || ap.x || ' ' || ap.y || ' ' || ap.z AS 'Teleport Location' FROM actor_position AS ap where class like '%human%' ORDER BY Thrall_ID DESC;
...
DROP VIEW IF EXISTS `Placeable_Item_Locations`;
CREATE VIEW `Placeable_Item_Locations` AS SELECT pb.name AS Owner, pn.name AS Type, COUNT(pn.object_id) AS 'Connected_Structures', 'TeleportPlayer ' || ap.x || ' ' || ap.y || ' ' || ap.z AS Location FROM properties AS pn INNER JOIN buildings b ON b.object_id = pn.object_id INNER JOIN actor_position ap ON ap.id = pn.object_id INNER JOIN ( SELECT guildid AS pb_id, name FROM guilds UNION SELECT id, char_name FROM characters ) pb ON b.owner_id = pb_id GROUP BY pn.object_id ORDER BY lower(Owner), COUNT(pn.object_id) DESC;
DROP VIEW IF EXISTS `Pets_Placed_In_World`;
CREATE VIEW `Pets_Placed_In_World` AS SELECT CASE WHEN class LIKE '%_pet_c%' THEN ap.id ELSE class END 'Pet_ID',CASE WHEN class LIKE '%_pet_c%' THEN ap.class ELSE class END 'Pet_Class', 'TeleportPlayer ' || ap.x || ' ' || ap.y || ' ' || ap.z AS Location FROM actor_position AS ap where class like '%_pet_c%' ORDER BY Pet_ID DESC;
...
CREATE VIEW `Detailed_Structure_Inventory` AS select quote(i.owner_id) as owner_id, cx.name as ItemName, quote(i.template_id) as template_id from item_inventory as i left outer join cust_item_xref as cx on i.template_id = cx.template_id WHERE inv_type = 4 order by i.owner_id;
DROP VIEW IF EXISTS `Detailed_Player_Stats`;
CREATE VIEW `Detailed_Player_Stats` AS select distinct c.char_name as Player_Name, quote(c.id) as Player_ID, cx.stat_name as Stat_Name, quote(cs.stat_value) as Stat_Value from character_stats as cs, characters as c left outer join cust_stat_xref as cx on cs.char_id = c.id WHERE cs.stat_id = cx.stat_id AND cs.stat_type = cx.stat_type order by cx.stat_type;
DROP VIEW IF EXISTS `Detailed_Player_Inventory`;
CREATE VIEW `Detailed_Player_Inventory` AS select i.owner_id as Player_Id, cx.name as Item_Name, quote(i.template_id) as Item_Id, c.char_name as player_name from characters as c, cust_item_xref as cx left outer join item_inventory as i on i.template_id = cx.template_id and i.owner_id = c.id WHERE inv_type = 0 or inv_type = 1 or inv_type = 2 order by c.id;
DROP VIEW IF EXISTS `Detailed_Player_Info`;
CREATE VIEW `Detailed_Player_Info` AS select g.name as GUILD, g.guildid as GUILDid, c.char_name as NAME, case c.rank WHEN '3' then 'Recruit' WHEN '2' then 'Leader' WHEN '1' then 'Officer' WHEN '0' then 'Member' ELSE 'No Clan' END RANK, c.level as LEVEL, (c.playerid), quote(c.id) as DBid, 'TeleportPlayer ' || ap.x || ' ' || ap.y || ' ' || ap.z as LOCATION, datetime(c.lastTimeOnline, 'unixepoch') as LASTONLINE from characters as c left outer join guilds as g on g.guildid = c.guild left outer join actor_position as ap on ap.id = c.id order by g.name, c.rank desc, c.level desc, c.char_name;
DROP VIEW IF EXISTS `Detailed_Game_Events`;
CREATE VIEW `Detailed_Game_Events` AS select datetime(ge.worldtime, 'unixepoch') as LocalHostTime, ge.eventType as EventType, 'TeleportPlayer ' || ge.x || ' ' || ge.y || ' ' || ge.z as IncidentArea, ge.ownerId as VictimPlayerId, ge.ownerName as VictimPlayerName, ge.ownerGuildId as VictimGuildID, ge.causerId as AttackingPlayerID, ge.causerName as AttackingPlayerName, ge.causerGuildId as AttackingGuildID, ge.causerGuildName as AttackingGuildName, ge.argsMap as ItemInQuestion, ge.stack as StackedAmount, ge.details as Details from game_events as ge order by EventType, AttackingPlayerID, AttackingPlayerName, AttackingGuildID, AttackingGuildName, LocalHostTime;
DROP VIEW IF EXISTS `Pippi_Merchants_Placed_In_World`;
CREATE VIEW `Pippi_Merchants_Placed_In_World` AS SELECT CASE WHEN class LIKE '%pippi%' THEN ap.id ELSE class END 'Merchant_ID',CASE WHEN class LIKE '%pippi%' THEN ap.class ELSE class END 'Pippi_Class', 'TeleportPlayer ' || ap.x || ' ' || ap.y || ' ' || ap.z AS 'Teleport Location' FROM actor_position AS ap where class like '%pippi%' ORDER BY Merchant_ID DESC;
